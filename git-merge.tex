...
Kad radimo s nekim projektom, onda nam je uvijek da imamo sačuvanu povijest projekta.
...
U tom slučaju -- sve međukorake kako je \verb+eksperimentalna-grana+ nastajala ($d$, $e$, $f$ i $g$), sve bi nestalo.

U nekim drugim situacijama, jednostavno želimo imati ljepši (linearni!) pregled povijesti projekta.
...
U svakom slučaju, ukoliko ste ga i započeli, uvijek se može prekinuti s:
...
Nećemo ovdje u detalje, ali recimo to ovako -- da, mogće je koristiti git kao klijent za druge (standardne) sustave za verzioniranje\footnote{Uz instalaciju posebnih \emph{plugin}ova, naravno.}.
...
Za normalno korištenje vrlo važno je da prije bilo kakvog \emph{commit}anja na te sustave -- svoje izmjene preselimo \emph{ispred} postojećih \emph{commit}ova.
