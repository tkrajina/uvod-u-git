...
SHA1 stringovi su računalu praktični, no ljudima u mrvicu nezgodni za pamćenje :)
...
Sve verzije svih datoteka, \emph{commit}ova i svega ostaloga se ovdje čuva.
...
\emph{Blob} sadrži binarne podatke neke datoteke, dok je \emph{tree} spisak datoteka.
...
Tada se u blob objekte može zapakirati više datoteka ili samo dijelova datoteka, ovisno o gitovom algoritmo za pakiranje (\emph{pack}).
...
Svaki od tih fajlova sarži referencu na jedan od objekata iz \verb+.git/objects+. 
Poigrajte se s \verb+git cat-file+ i otkriti ćete da su to uvijek \emph{commit} objekti.

Zaključatk se sam nameće -- u \verb+.git/refs+ se nalaze reference na sve grane, tagove i grane udaljenih repozitorija.
...
I to na onu od tih datoteka koja sadrži referencu na onu granu u kojoj se trenutno nalazimo.
...
Svaki git repozitorij več sadrži primjere takvih skripti s ekspenzijom \verb+.sample+.
